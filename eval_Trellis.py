...
def main():
    ...
    for idx in valid_indices:
        ...
        single_view = np.array([[90, 270.0, 0.0]], dtype=np.float32)  
        ref_images_list = []
        for j_img, sample_3d in enumerate(trellis_objects):
            ...
            for i_img in range(n_images):
                ...
                for j_img in range(n_images):
                    if i_img == j_img:
                        ...
                        l2_coord_matches[i_img][j_img] = np.array([90, 270.0, 0.0])
                        lpips_coord_matches[i_img][j_img] = np.array([90, 270.0, 0.0])
                        clip_coord_matches[i_img][j_img] = np.array([90, 270.0, 0.0])
                        dino_coord_matches[i_img][j_img] = np.array([90, 270.0, 0.0])

...
def apply_camera_roll(extr, psi_deg):
    roll_rad = torch.tensor(math.radians(psi_deg), device='cuda')
...
def extrinsics_intrinsics_from_coords(camera_coords, radius=1.7, fov_deg=40.0, device="cuda"):
    """Generate extrinsics and intrinsics matrices from camera coordinates."""
    extrs = []
    intrs = []
...
def main():
    ...
    # Grab a specific example
    target_idx = None
    for i, ex in enumerate(mochi_all):
        dataset = ex["dataset"]
        trial   = ex["trial"]
        if dataset == "barense" and trial == "familiar_high_screen02":
            target_idx = i
            break
    ...
    out_base = Path("data/trellis_rotation_test_canonical7")
    ...
    print("Rendering 3D models from the Trellis canonical viewpoint...")


    canonical_view = np.array([[90, 270.0, 0.0]], dtype=np.float32)  # (theta, phi, roll)
    radius = 2.0  
    fov_deg = 40.0
    ...
    for i, model in enumerate(trellis_objects):
        ...
        mediapy.write_image(reconstructed_dir / f"canonical_from_rotated_{angle}.png", color_img)
